...
public class ZipCodeIntervalConfigUtil {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ZipCodeIntervalConfigUtil.class);
	...
	/**
	 * This method is used to convert the currentIntervals to miniumRangeOfIntervals.
	 * For e.g 
	 * Input = [94133,94133] [94200,94299] [94226,94399] 
     * Then the output should be = [94133,94133] [94200,94399]
	 * The method will not alter the state of the input List.
	 * 
	 * @param currentIntervals
	 * @return
	 */
	public List<Interval> calculateMinimumRangeOfIntervals(List<Interval> currentIntervals) {
		...
		Collections.sort(inputCopyList,Comparator.comparing(Interval::getStartingVal));
		...
		for (int i = 1; i < inputCopyList.size(); i++) {
			...
			if (currentIntervalEnding >= nextInterval.getStartingVal()) { 
				currentIntervalEnding = Math.max(nextInterval.getEndingVal(),currentIntervalEnding); 
			} else {
				minimumRangeOfIntervals.add(new Interval(currentIntervalStarting, currentIntervalEnding));
